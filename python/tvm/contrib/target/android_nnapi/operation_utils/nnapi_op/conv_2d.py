...
def add_operation(converter, inputs, outputs): 
    """Add an ANEURALNETWORKS_CONV_2D operation with checking
    ...
    ANC_COMPATIBILITY_CHECK(API_LEVEL >= 27, f"Target Android API level { API_LEVEL } is too low to support the operation")

    # check inputs
    if (API_LEVEL >= 29): 
        ANC_NNAPI_OP_CHECK(len(inputs) == 13)
    else: 
        ANC_NNAPI_OP_CHECK(len(inputs) == 10)
    ...
    if (ins[1]["dtype"] == "TENSOR_FLOAT16"): 
        ...
        ANC_NNAPI_OP_CHECK(ins[1]["dtype"] == "TENSOR_FLOAT32")
    ...
    felter = { k: v for (k, v) in zip([ "do", "fh", "fw", "di" ], ins[1]["shape"]) }
    
    # check inputs[2]
    ins[2] = {}
    ins[2]["dtype"] = converter.export_obj.helper.operand.get_dtype(inputs[2])
    ANC_NNAPI_OP_CHECK(ins[2]["dtype"] == ins[1]["dtype"])
    ...
    # check inputs[9]
    ANC_NNAPI_OP_CHECK(converter.export_obj.helper.operand.is_FuseCode(inputs[9]))

    if (API_LEVEL >= 29): 
        # check inputs[10]
        ins[10] = {}
        ins[10]["dtype"] = converter.export_obj.helper.operand.get_dtype(inputs[10])
        ANC_NNAPI_OP_CHECK(ins[10]["dtype"] == "BOOL")
        ins[10]["value"] = converter.export_obj.helper.operand.get_value(inputs[10])
        ANC_NNAPI_OP_CHECK(ins[10]["value"] == "false" or ins[10]["value"] == "true")

        # check inputs[11]
        ins[11] = {}
        ins[11]["dtype"] = converter.export_obj.helper.operand.get_dtype(inputs[11])
        ANC_NNAPI_OP_CHECK(ins[11]["dtype"] == "INT32")
        ins[11]["value"] = converter.export_obj.helper.operand.get_value(inputs[11])
        ANC_NNAPI_OP_CHECK(ins[11]["value"] >= 1)

        # check inputs[12]
        ins[12] = {}
        ins[12]["dtype"] = converter.export_obj.helper.operand.get_dtype(inputs[12])
        ANC_NNAPI_OP_CHECK(ins[12]["dtype"] == "INT32")
        ins[12]["value"] = converter.export_obj.helper.operand.get_value(inputs[12])
        ANC_NNAPI_OP_CHECK(ins[12]["value"] >= 1)
    ...
    ANC_NNAPI_OP_CHECK(felter["di"] == data_shape["c"])
    ...
    converter.export_obj.add_operation("CONV_2D", inputs, outputs)
