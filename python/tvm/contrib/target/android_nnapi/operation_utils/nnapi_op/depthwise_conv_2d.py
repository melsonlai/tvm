...
def add_operation(converter, inputs, outputs): 
    ...
    # check inputs
    if (API_LEVEL >= 29): 
        ANC_NNAPI_OP_CHECK(len(inputs) == 14)
    else: 
        ANC_NNAPI_OP_CHECK(len(inputs) == 11)
    ...
    felter = { k: v for (k, v) in zip([ "di", "fh", "fw", "do" ], ins[1]["shape"]) }
    ANC_NNAPI_OP_CHECK(felter["di"] == 1)
    ...
    ANC_NNAPI_OP_CHECK(depth_multiplier >= 0)
    ...
    if (API_LEVEL >= 29): 
        ...
        ANC_NNAPI_OP_CHECK(ins[13]["dtype"] == "INT32")
        ins[13]["value"] = converter.export_obj.helper.operand.get_value(inputs[13])
        ANC_NNAPI_OP_CHECK(ins[13]["value"] >= 1)
    ...
    ANC_NNAPI_OP_CHECK(felter["do"] == data_shape["c"] * depth_multiplier)
