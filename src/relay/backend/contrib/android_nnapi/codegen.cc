...
class CodegenNNAPI : public backend::MemoizedExprTranslator< ::std::vector< Output > >, public CodegenCBase {
 public:
  ...
  ::std::vector< Output > VisitExpr_(const FunctionNode * func) final {
    ...
    /* generate function body */
    {
      ...
      const auto * pf = backend::GetPackedFunc("relay.ext.android_nnapi_compiler.relayir_to_nnapi_converter");
      ICHECK(pf) << "Cannot find relay.ext.android_nnapi_compiler.relayir_to_nnapi_converter";
    ...
    ICHECK(func_ref->body->checked_type()->IsInstance< TensorTypeNode >()) << "Expects single output Function to be converted for NNAPI";
    ...
    out.need_copy = true;
    {
      ...
      buf_stream << out.dtype << " * " << out.name << " = static_cast< " << out.dtype << " * >(::std::malloc(" << out_ttype->dtype.bytes() * out.size << "));";
    ...
    /* generate call to the generated function */
    {
      ...
      for (int i = 0; i < func_ref->params.size(); ++i) {
  ...
  ::std::string JIT(const ::std::vector< Output > & out) {
    ...
    return JitImpl(this->ext_func_id_, this->ext_func_args_, this->buf_decl_, this->ext_func_body_, this->const_array_name_, out);
  }
...
TVM_REGISTER_GLOBAL("relay.ext.android_nnapi_compiler").set_body_typed(CCompiler);
